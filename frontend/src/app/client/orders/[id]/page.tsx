  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-blue-950 to-slate-900 text-white font-sans">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        {/* ✅ НОВОЕ: Подтверждение медика */}
        {order.status === 'ACCEPTED' && !order.confirmedByClient && (
          <div className="mb-6 p-6 rounded-2xl bg-gradient-to-r from-yellow-500/10 to-orange-500/10 border-2 border-yellow-500/30">
            <div className="flex items-start space-x-4 mb-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-slate-300 mb-3">
                  Медик <span className="font-semibold text-white">{order.medic?.name}</span> принял ваш заказ.
                  Подтвердите, чтобы медик мог выехать к вам.
                </p>
                
                {/* Информация о медике */}
                <div className="bg-white/5 rounded-xl p-3 mb-3 space-y-2 text-sm">
                  <div className="flex items-center justify-between">
                    <span className="text-slate-400">Телефон:</span>
                    <a href={`tel:${order.medic?.phone}`} className="text-cyan-400 hover:text-cyan-300">
                      {order.medic?.phone}
                    </a>
                  </div>
                  <div className="flex items-center justify-between">

...
export default function MedicProfilePage() {
  ...
  useEffect(() => {
    // Мок-данные для примера, так как у нас нет доступа к API
    const mockMedicData = {
      id: medicId,
      ...
      experience: 12,
      completedOrders: 345,
      bio: "Опытный терапевт с 12-летним стажем. Специализируюсь на диагностике и лечении внутренних болезней. Подхожу к каждому пациенту индивидуально, используя современные методы лечения.",
      education: "Казахский национальный медицинский университет им. С.Д. Асфендиярова (2012 г.)",
      services: ["Консультация на дому", "Расшифровка анализов", "Назначение лечения", "Медицинский осмотр"],
      phone: "+77012345678",
      ratingDistribution: {
        "5": 120,
        "4": 5,
      ...
      reviews: [
        {
          id: "r1",
          clientName: "Айгуль",
          serviceType: "Консультация на дому",
          rating: 5,
          createdAt: "2024-10-20T10:30:00Z",
          comment: "Екатерина - замечательный врач! Очень внимательная, все подробно объяснила и назначила эффективное лечение. Быстро пошла на поправку. Спасибо!"
        },
        {
          id: "r2",
          clientName: "Марат",
          serviceType: "Расшифровка анализов",
          rating: 5,
          createdAt: "2024-10-18T14:00:00Z",
          comment: "Получил быструю и понятную расшифровку анализов. Врач дала рекомендации. Очень удобно."
        },
        {
          id: "r3",
          clientName: "Елена",
          ...
          comment: "Хороший специалист. Помогла с моей проблемой. Немного опоздала, но в целом я довольна."
        }
      ]
    };
    ...
    const loadMedicProfile = async () => {
      setLoading(true);
      try {
        // Закомментировано для использования мок-данных.
        ...
        <div className="rounded-2xl bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl border border-white/20 p-8 mb-6">
          <div className="flex flex-col sm:flex-row items-start justify-between mb-6 gap-4">
            <div className="flex items-start space-x-4">
              <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-full bg-gradient-to-br from-cyan-400 to-blue-600 flex items-center justify-center text-3xl sm:text-4xl font-bold shadow-xl flex-shrink-0">
                {medic.name[0]}
              </div>
              <div>
                <h1 className="text-2xl sm:text-3xl font-bold mb-2">{medic.name}</h1>
                <p className="text-cyan-400 text-md sm:text-lg mb-2">{medic.specialization}</p>
          ...
          {medic.bio && (
            <div className="mb-6">
              ...
              <p className="text-slate-300 whitespace-pre-wrap">{medic.bio}</p>
            </div>
          )}
            ...
            <div className="mb-6">
              ...
              <div className="flex flex-wrap gap-2">
                {medic.services.map((service: string, index: number) => (
                  <span
                    key={index}
                    className="px-4 py-2 rounded-full bg-cyan-500/20 border border-cyan-500/30 text-cyan-400 text-sm"
                  >
                    {service}
                  </span>
                ))}
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            {/* ИСПРАВЛЕНИЕ: Добавлен открывающий тег <a> */}
            <a
            ...
            <div key={star} className="flex items-center gap-3 mb-2">
              ...
              <div className="flex-1 h-2 rounded-full bg-white/10 overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${medic.reviewCount > 0 ? ((medic.ratingDistribution[star] || 0) / medic.reviewCount) * 100 : 0}%`
                  }}
                />
              </div>
              <span className="text-sm text-slate-400 w-12 text-right">
                {medic.ratingDistribution[star] || 0}
              </span>
            </div>
          ))}
        </div>

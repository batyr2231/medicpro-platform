...
  // ВОЗВРАЩАЕМ реальные хуки
  const params = useParams();
  const router = useRouter();
  const orderId = params.id as string;
  
  const [order, setOrder] = useState<any>(null);
  // ВОЗВРАЩАЕМ реальный хук useOrders
...
  useEffect(() => {
    // ❗️❗️❗️ ИСПРАВЛЕНИЕ: Не запускать, если orderId ЕЩЕ не определен
    if (!orderId) {
      return;
    }
...
  // ВОЗВРАЩАЕМ реальную функцию loadOrder
  const loadOrder = async () => {
    try {
      // Дополнительная проверка
      if (!orderId) return;

      // Проверка, чтобы избежать "гонки состояний" из-за setInterval
      if (loading && order) return; // (Если уже грузится И есть данные, не перезагружать)
...
        {/* Блок отзыва - (ВОЗВРАЩАЕМ РЕАЛЬНУЮ ЛОГИКУ) */}
...
        {/* Cancel Button (ВОЗВРАЩАЕМ РЕАЛЬНУЮ ЛОГИКУ) */}
        {order.status === 'NEW' && (
          <button
            onClick={async () => {
              try {
                const token = localStorage.getItem('token');
                // Убедитесь, что NEXT_PUBLIC_API_URL доступен, или используйте fallback
                const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'https://medicpro-platform.onrender.com';

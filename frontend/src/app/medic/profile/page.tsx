...
export default function MedicProfilePage() {
  ...
  const [selectedDocType, setSelectedDocType] = useState<'LICENSE' | 'CERTIFICATE' | null>(null);
...
const handleDisconnectTelegram = async () => {
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>, type: 'LICENSE' | 'CERTIFICATE') => {
  const file = e.target.files?.[0];
  if (!file) return;
...
const handleUpload = async (type: 'LICENSE' | 'CERTIFICATE') => {
  ...
  try {
    ...
    toast.success(`✅ ${type === 'LICENSE' ? 'Лицензия' : 'Сертификат'} загружен!`);
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div className="text-sm text-cyan-300">
                    ...
                    <ul className="list-disc list-inside space-y-1 text-cyan-400/80">
                      <li>Медицинская лицензия (обязательно)</li>
                      <li>Сертификаты и дипломы (по желанию)</li>
                    </ul>
